...
/**
 * Motor de precificação sacaria — replica exata da planilha:
 * - CUSTO RÁFIA = 2 × largura × comprimento × (gramatura/1000) × preço_kg
 * - CUSTO LINHA, CUSTO CORTE, CUSTO IMPRESSO (0,125 por lado)
 * - c (liso) = RÁFIA + LINHA + CORTE
 * - VALOR = (custo / inverso) / 0,915  (imposto NF 8,5%)
 * - Faixas: <500 → i=0,55; 1000+ → i=0,65; 2000+ → i=0,70
 */

export interface RawPricingParams {
  width: number;
  length: number;
  grammage: number;
...
export interface PlanilhaExtrasDoc {
  bagLineCm?: number;
  lineWeightPerCm?: number;
  linePricePerGram?: number;
}

export interface PricingParamsDoc {
  ...
  planilhaExtras?: PlanilhaExtrasDoc;
  validFrom?: admin.firestore.Timestamp | { toMillis: () => number };
}

/** Custo linha por unidade (R$/saco): cada sacaria passa (cm) × peso linha 1 cm (g) × preço/g */
function computeLineCostFromPlanilha(planilha: PlanilhaExtrasDoc | undefined): number {
  if (!planilha?.bagLineCm || !planilha?.lineWeightPerCm || !planilha?.linePricePerGram) return 0;
...
export interface PricingResult {
  ...
  marginAmount: number;
  liquidProfit?: number;
  steps: Array<{ name: string; input?: unknown; output: number }>;
  valorLiso?: number;
  valorFrente?: number;
  valorFrenteVerso?: number;
...
const DEFAULT_INVERSE_BANDS: QuantityBand[] = [
...
function getBands(paramsSnap: admin.firestore.QuerySnapshot): QuantityBand[] {
  if (paramsSnap.empty) return DEFAULT_INVERSE_BANDS;
  const bands = (paramsSnap.docs[0].data() as PricingParamsDoc).quantityBands;
  if (bands?.length) return bands.filter((b) => b.inverse != null);
  return DEFAULT_INVERSE_BANDS;
}

function getInverseForQuantity(quantity: number, bands: QuantityBand[] | undefined): number {
  if (!bands?.length) return 0.65;
  const band = bands
...
export async function calculatePrice(
  ...
  if (!qty || qty <= 0) throw new Error('quantity is required and must be positive');

  let widthCm: number;
  let lengthCm: number;
  let grammage: number;
  let materialId: string;
  let printType: 'liso' | 'frente' | 'frente_verso';
  ...
  if (rawParams) {
    ...
    if (!variantSnap.exists) throw new Error('Product variant not found');
    const variant = variantSnap.data() as VariantDoc;
    ...
    printType = (variant.printTypeId === 'frente_verso' ? 'frente_verso' : variant.printTypeId === 'frente' ? 'frente' : 'liso') as 'liso' | 'frente' | 'frente_verso';

    if (variant.grammageValue != null) grammage = variant.grammageValue;
    else if (template.grammageId) {
      const gramSnap = await db.collection('grammages').doc(template.grammageId).get();
      grammage = (gramSnap.data() as { value: number })?.value ?? 0;
    } else grammage = 0;
  } else {
  ...
  if (!paramsSnap.empty) {
    const params = paramsSnap.docs[0].data() as PricingParamsDoc;
    if (params.rafiaPricePerKg != null) rafiaPricePerKg = params.rafiaPricePerKg;
    lineCost = params.lineCost != null && params.lineCost > 0
      ? params.lineCost
      : computeLineCostFromPlanilha(params.planilhaExtras);
    if (params.cutCost != null) cutCost = params.cutCost;
    if (params.printCostPerSide != null) printCostPerSide = params.printCostPerSide;
    if (params.taxFactor != null) taxFactor = params.taxFactor;
  }

  if (materialId) {
    const materialSnap = await db.collection('materials').doc(materialId).get();
    if (materialSnap.exists) {
      const costKg = (materialSnap.data() as { costPerKg?: number })?.costPerKg;
      if (costKg != null) rafiaPricePerKg = costKg;
    }
  }
